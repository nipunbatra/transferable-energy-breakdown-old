...
def create_df_main(appliance, year, train_regions, train_fraction_dict,
	...
	for train_region in train_regions:
		...
		temp_df = temp_df.ix[[x for x in temp_df.index if x not in test_home_list]]
	...
	test_df = test_df.ix[test_home_list]
	test_dfc = test_dfc.ix[test_home_list]
...
features_dict['region'] = ['dd_1', 'dd_2', 'dd_3', 'dd_4', 'dd_5', 'dd_6', 'dd_7', 'dd_8', 'dd_9', 'dd_10', 'dd_11',
                           'dd_12']
...
def find_valid_homes(region, appliance, appliance_fraction, aggregate_fraction):
	...
	# Condition #1
	valid_homes_aggregate = df.ix[
		df[aggregate_cols].notnull().sum(axis=1) >= aggregate_num_cols * aggregate_fraction].index

	# Condition #2
	valid_homes_appliance = df.ix[
		df[appliance_cols].notnull().sum(axis=1) >= appliance_num_cols * appliance_fraction].index
