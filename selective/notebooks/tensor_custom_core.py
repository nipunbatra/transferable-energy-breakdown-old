...
def cost_graph_laplacian(H, A, T, L, E_np_masked, lam, weight_matrix, case):
    HAT = multiply_case(H, A, T, case)
    mask = ~np.isnan(E_np_masked)
    error_1 = np.multiply((HAT - E_np_masked), weight_matrix)[mask].flatten()
...
def learn_HAT_adagrad_graph(case, tensor, L, num_home_factors, num_season_factors, weight_matrix, num_iter=2000, lr=0.01, dis=False,
    ...
    Ts = [T.copy()]
    As = [A.copy()]
    costs = [cost(H, A, T, L, tensor, lam, weight_matrix, case)]
    HATs = [multiply_case(H, A, T, case)]

    # GD procedure
    for i in range(num_iter):
        del_h, del_a, del_t = mg(H, A, T, L, tensor, lam, weight_matrix, case)
        ...
        costs.append(cost(H, A, T, L, tensor, lam, weight_matrix, case))

        HATs.append(multiply_case(H, A, T, case))
        if i % 500 == 0:
            if dis:
                print(cost(H, A, T, L, tensor, lam, weight_matrix, case))
