		...
		for static_fac in ['static']:
			for lam in [0]:
				for random_seed in range(10):
					...
					SLURM_SCRIPT = "%s/out_%d_%s_%s_%f_%d.pbs" % ("./pbs_file", train_iter, algo, static_fac, lam, random_seed)
					CMD = 'python transfer_adagrad_static_sparse.py %d %s %s %f %d' % (train_iter, algo, static_fac, lam, random_seed)
					lines = []
					lines.append("#!/bin/sh\n")
