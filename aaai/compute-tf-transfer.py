...
for train_percentage in [6, 8, 10,12, 14,16,18, 20, 30, 40, 50, 60, 70,80, 90, 100]:
	...
	for random_seed in range(5):
		...
		directory = os.path.expanduser('~/git/pred_tf/{}_to_{}/'.format(source, target))

		filename = os.path.expanduser('~/git/pred_tf/{}_to_{}/'.format(source, target) + name + '.pkl')
...
pickle.dump(out, open("predictions/{}-{}-tf-transfer-cv.pkl".format(source, target),"w"))
