		...
		for cost in ['abs', 'rel']:
			for a in [2, 3]:
				for lr in [0.01, 0.1, 1, 10, 100]:
					OFILE = "%s/con_%s_%s_%s_%d_%f.out" % (SLURM_OUT, region, algo, cost, a, lr)
					EFILE = "%s/con_%s_%s_%s_%d_%f.err" % (SLURM_OUT, region, algo, cost, a, lr)
					SLURM_SCRIPT = "%s/con_%s_%s_%s_%d_%f.pbs" % ("./pbs_file", region, algo, cost, a, lr)
					CMD = 'python check_convergency.py %s %s %s %d %f' % (region, algo, cost, a, lr)
					lines = []
					lines.append("#!/bin/sh\n")
