...
for lam in [0.001, 0.01, 0.1, 0, 1]:
	OFILE = "{}/{}-graph.out".format(SLURM_OUT, lam)
	EFILE = "{}/{}-graph.err".format(SLURM_OUT, lam)
	SLURM_SCRIPT = "{}/{}-graph.pbs".format('pbs_files', lam)
	CMD = 'python graph_laplacian.py {} {} {}'.format(lam)
	lines = []
	lines.append("#!/bin/sh\n")
